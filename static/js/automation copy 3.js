...
let isSportsRadarRunning = false;
...
const sportsradarBtn = document.getElementById("sportsradarBtn");
...
const interval = setInterval(async () => {
    ...
    if (logs.at(-1)?.includes("✅ Job complete")) {
        ...
        localStorage.removeItem("sportsRadarRunning");
        localStorage.removeItem("sportsRadarJobId");
        ...
        sportsradarBtn.disabled = false;
        sportsradarBtn.textContent = " Start Sportsradar"
    }
...
const startSportsRadarProcessAutomation = async (formData) => {
    if (isSportsRadarRunning || localStorage.getItem("sportsRadarRunning") === "true") {
        console.log("Sportsradar is already running.")
        return;
    }
    const {
        selectedBrand,
    ...
    isSportsRadarRunning = true;
    localStorage.setItem("sportsRadarRunning", "true");

    sportsradarBtn.disabled = true;
    sportsradarBtn.textContent = "Processing...";

    try {
        const res = await fetch("/api/v1/sportsradar/start", {
            method: "POST",
            headers: {
                "Content-Type": "application/json"
            },
        ...
        const data = await res.json();
        console.log("Sportsradar Done: ", data);
        if (data.job_id) {
            localStorage.setItem("sportsRadarJobId", data.job_id);
            pollLogs(data.job_id, "sportsradar");
        }

        console.log("sportsradar Started Succesfully..");
        localStorage.removeItem("sportsRadarRunning")
    } catch (err) {
        console.error("Sportsradar  Error: ", err);
        console.log("Sportsradar Failed,");
        localStorage.removeItem("sportsRadarRunning")
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    const sportsRadarRunning = localStorage.getItem("sportsRadarRunning") === "true";
    ...
    if (sportsRadarRunning) {
        sportsradarBtn.disabled = true;
        sportsradarBtn.textContent = "Processing...";
        isSportsRadarRunning = true;

        const jobId = localStorage.getItem("sportsRadarJobId");
        if (jobId) pollLogs(jobId, "sportsradar")
    }

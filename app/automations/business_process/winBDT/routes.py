...
# ────────────────────── SCHEMA ────────────────────────
class SportsRadarInput(BaseModel):
...
@router.post("/start")
def start_automation(data: SportsRadarInput, background_tasks: BackgroundTasks):
    job_id = str(uuid.uuid4())
    job_logs[job_id] = ["🟡 Job accepted. Waiting to start..."]  # init
    background_tasks.add_task(run, job_id, data.brand, data.currency, data.startDate, data.endDate, data.timeGrain)

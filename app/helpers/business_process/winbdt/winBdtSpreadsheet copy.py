...
class winBdtSpreadsheet():
    ...
    def insert_account_creation(self, copy_url, values):
        """
        Insert AccountCreation data separately (preserve leading zeros without breaking formulas).
        """
        try:
            if not values:
            ...
            def protect_value(val):
                val_str = str(val)
                # If it's numeric with leading zeros → wrap in formula ="..."
            ...
            start_row = self.get_first_empty_row(copy_url, WINBDT_RANGE["AccountCreation"], col="A", start_row=1)

            body = {
                "valueInputOption": "USER_ENTERED",  # allow formula ="0123"
                "data": [{

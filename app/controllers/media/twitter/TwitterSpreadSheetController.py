...
class TwitterSpreadsheetController:
    ...
    def _get_value_from_column_e(self, job_id, values, currency_row_index, currency):
        """
        Extract Twitter metrics starting from the given currency_row_index
        using column E (index 4).
        """
        log(job_id, "Getting value from column E...")

        def safe_get(offset):
            row_index = (currency_row_index + offset) - 1
            try:
                return float(values[row_index][4].replace(',', ''))
            except (IndexError, ValueError, AttributeError):
                log(job_id, f"Invalid or missing value at row {row_index + 1}")
                return None
        
        return {
            "followers":        safe_get(0),  # TOTAL FOLLOWERS
            "gain":             safe_get(1),  # DAILY FOLLOWERS GAIN
            "daily_engaged":    safe_get(2),  # DAILY ENGAGEMENTS
            "monthly_engaged":  safe_get(3),  # MONTHLY ENGAGEMENTS
            "yearly_engaged":   safe_get(4),  # TOTAL ENGAGEMENTS
            "daily_views":      safe_get(5),  # DAILY VIEWS
            "monthly_views":    safe_get(6),  # MONTHLY VIEWS
            "yearly_views":     safe_get(7),  # TOTAL VIEWS
        }
    ...
    def _update_sheet_values(self, job_id, service, spreadsheet_id, sheet_id, tab_name, 
                            today_str, currency_row_index, insights, total_followers, value_in_column_objects):
        ...
        data = [
            {
                "range": f"{tab_name}!D{currency_row_index-1}:D{currency_row_index+9}",
                "values": all_values
            }
        ]
        ...
        log(job_id, f"Updated values and formatted rows {currency_row_index-1} to {currency_row_index+9}")
        return values_only

    def get_twitter_spreadsheet_column(self, job_id, spreadsheet_id: str, tab_name: str, currency: str, insights:list, total_followers: int = 0 ,page_type: str = "page"):
        try:
            # Initialize service
            ...
            value_in_column_objects = self._get_value_from_column_e(job_id, values, currency_row_index, currency)
            if value_in_column_objects is None:
                return None

            # Update cells with today's date, total followers, and difference
            formated_values = self._update_sheet_values(
    ...
    def calculate_day_deltas(self, job_id, post_age, insights, existing_row):
        ...
        views_3 = safe_int(existing_row[3]) if len(existing_row) > 3 else 0
        views_7 = safe_int(existing_row[4]) if len(existing_row) > 4 else 0
        ...
        views_now = insights.get('views', 0)
        ...
        if post_age >= 3:
            result["views_3"] = str(views_3) if views_3 > 0 else str(views_now)
        ...
        if post_age >= 7:
            ...
            result["views_7"] = str(max(views_now - views_3, 0)) if views_3 else ''
        ...
        if post_age >= 30:
            result["views_3"] = str(views_3) if views_3 > 0 else str(views_now)
            result["views_7"] = str(views_7) if views_7 > 0 else str(max(views_now - views_3, 0)) if views_3 else ''
            result["views_30"] = str(max(views_now - (views_3 + views_7), 0)) if (views_3 or views_7) else ''
    ...
    def transfer_timeline_insight_data(self, job_id, spreadsheet_id: str, tab_name: str, insights_data: list, followers: dict, date: str = None):
        try:
            ...
            for post in insights_data:
                ...
                date_str = post.get('created_at', yesterday) # fallback to yesterday if missing
                ...
                incoming_post_id = post.get("tweet_id", '')
                # Match rows where:
                deltas = {
                    "views_3": '',
                    "views_7": '',
                ...
                # print(deltas)
                if post_age == 3 and deltas['views_3'] == '':
                    deltas['views_3'] = str(insights.get('views', ''))
                    # deltas['imp_3'] = str(insights.get('impressions', ''))
                    deltas['react_3'] = str(insights.get('reactions', ''))

                if post_age == 7 and deltas['views_7'] == '':
                    deltas['views_7'] = str(insights.get('views', ''))

...
class YoutubeSpreadsheetController:
    ...
    def _update_sheet_values(self, job_id, service, spreadsheet_id, sheet_id, tab_name, 
        ...
        values_only = [
            ...
            insights.get('daily_insights', {}).get('views'),  # DAILY IMPRESSIONS
            insights.get('monthly_insights', {}).get('views'),  # MONTHLY IMPRESSIONS
            insights.get('yearly_insights', {}).get('views'),  # TOTAL IMPRESSIONS
            insights.get('daily_insights', {}).get('engagedViews'),  # DAILY REACH
            insights.get('monthly_insights', {}).get('engagedViews'),  # MONTHLY REACH
            insights.get('yearly_insights', {}).get('engagedViews')   # TOTAL REACH
        ]
    ...
    def get_youtube_spreadsheet_column(self, job_id, spreadsheet_id: str, tab_name: str, currency: str, insights:list, total_followers: int = 0 ,page_type: str = "page"):
        try:
            # Initialize service
    ...
    def transfer_video_insight_data(self, job_id, spreadsheet_id: str, tab_name: str, insights_data: list, followers: dict, date: str = None):
        try:
            ...
            for post in insights_data:
                insights = {
                    "reach": post.get('engaged_views', 0),
                ...
                date_str = post.get('published_at', yesterday) # fallback to yesterday if missing
                ...
                incoming_post_id = post.get("video_id", '')
                # Match rows where:
                deltas = {
                    "reach_3": '',
                    "reach_7": '',
            ...
            if e.resp.status == 429:
                ...
                time.sleep(60)
                return self.transfer_video_insight_data(job_id, spreadsheet_id, tab_name, insights_data, followers, date)
            return False
        except Exception as e:
